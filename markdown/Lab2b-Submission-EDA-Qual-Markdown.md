...
- [The repeated pre-processing
  includes:](#the-repeated-pre-processing-includes)
...
blue_grey_colours_11 \<- c(“\#27408E”, “\#304FAF”, “\#536CB5”,
“\#6981c7”, “\#8da0db”, “\#dde5ec”, “\#c8c9ca”, “\#B9BCC2”, “\#A7AAAF”,
“\#888A8E”, “\#636569”)

blue_grey_colours_6 \<- c(“\#27408E”, “\#304FAF”, “\#536CB5”,
“\#B9BCC2”, “\#A7AAAF”, “\#888A8E”)

blue_grey_colours_4 \<- c(“\#27408E”, “\#536CB5”, “\#B9BCC2”,
“\#888A8E”)
...
“dashed”, lineend = NULL, color = “\#dfdede”, arrow = NULL,
inherit.blank = FALSE), panel.background = element_rect(fill =
“\#f3eeee”), legend.title = element_text(face = “plain”, color =
“\#3f3f41”, size = 12, hjust = 0), legend.position = “right” ) }

# Customize the text tables for consistency using HTML formatting

kable_theme \<- function(dat, caption) { kable(dat, “html”, escape =
...
correct_spelling \<- function(x) { sapply(1:length(x), function(y) { bad
\<- hunspell(x\[y\])\[\[1\]\] good \<-
...
and wishes. {r Your Eighth Code Chunk} \## Evaluation Likes —- \### Word
count per gender —- word_count_per_gender_likes \<-
...
facet_wrap(~`Class Group`, scales = “free”) + scale_x_continuous( breaks
= popular_words$row,  labels = popular_words$`Likes (tokenized)`) +
coord_flip()
...
“Word in Course Evaluation”, y = “Number of Times Used (Term
Frequency)”) + ggtitle(“Most Frequently Used Words in Course Evaluation
Wishes per Class Group”) + facet_wrap(~`Class Group`, scales = “free”) +
scale_x_continuous( breaks =
popular_words$row,  labels = popular_words$`Wishes (tokenized)`) +
coord_flip()
...
evaluation_likes_filtered %\>% count(`Likes (tokenized)`, sort = TRUE)
...
{r Your Tenth Code Chunk} \## Evaluation Likes —- \### TF-IDF Score per
Gender —- popular_tfidf_words_gender_likes \<- evaluation_likes_filtered
...
`Student's Gender`, `Average Course Evaluation Rating`,
`Likes (tokenized)`) %\>% count(`Student's Gender`, `Likes (tokenized)`,
sort = TRUE) %\>% ungroup() %\>% bind_tf_idf(`Likes (tokenized)`,
`Student's Gender`, n)
...
Xie \[aut, Y., cre, Sarma, A., Vogt, A., Andrew, A., Zvoleff, A.,
